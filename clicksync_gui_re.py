...
class ClickSyncGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("ClickSync - Tkinter GUI (커서복귀 추가)")
        self.running = False
        self.arduino = None
                    ...
                    if sub_x is not None:
                        ...
                        if self.arduino:
                            try:
                                ...
                                self.log(f"Sent to Arduino: {msg.strip()} (waiting for OK...)")
                                # 2. OK 신호 대기(최대 2초)
                                ok = ""
                                try:
                                    ok = self.arduino.readline().decode(errors='ignore').strip()
                                except Exception:
                                    ok = ""
                                if ok == "OK":
                                    # 3. 커서를 원래 위치로 복귀
                                    pyautogui.moveTo(cur_x, cur_y)
                                    self.log(f"[PY] 마우스 커서 원래 위치로 복귀: ({cur_x},{cur_y})")
                                else:
                                    self.log(f"[ERR] 아두이노 OK 수신 실패: [{ok}]")
                            except Exception as e:
